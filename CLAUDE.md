...
15. ✅ **`rgx-mcp` v0.1.0** — lives **in this repo** at `tools/rgx-mcp/` and **ships inside the packaged player zip** since v2.4.0 (owner's call: users who install the framework and want to learn addon development already have the MCP server on hand; `.pkgmeta` keeps `docs/` and `schemas/` excluded but not `tools/`). Read-only stdio MCP server, verified over live JSON-RPC: `rgx_validate_addon` (reads the schema live from the enclosing checkout — one source of truth), `rgx_audit_lua` (deterministic unsafe-pattern detectors), `rgx_generate_addon` (shipped keys only, `dbName` override for hyphenated names), `rgx_get_contract` + `rgx://` resources. Registered in `.mcp.json`, so Claude Code sessions here get the `rgx_*` tools (run `npm install` in `tools/rgx-mcp/` once). End-to-end test: `tools/rgx-mcp/test/test-rgx-hello.mjs` drives the real server against the real RGX-Hello repo (generate/validate/audit) — re-run it after touching the schema, the declarative options mapper in `core/core.lua`, or the generator; its first run caught the silently-dropped slider `suffix` and the hyphenated-dbName generator gap (fixed in v2.4.1). Dependency direction enforced: the tool reads RGX docs/schema; the addon runtime never references `tools/`. The old separate repo `DonnieDice/rgx-mcp` is archived with a moved notice. Tier 4 implementation should update the schema annotations (`tier4` → `today`) as each key lands — nothing else changes.
