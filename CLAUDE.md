...
15. ✅ **`rgx-mcp` v0.1.0** — lives **in this repo** at `tools/rgx-mcp/` (owner's call: everyone with the framework checkout has the tool; `.pkgmeta` excludes `tools/` from the player zip). Read-only stdio MCP server, verified over live JSON-RPC: `rgx_validate_addon` (reads the schema live from the enclosing checkout — one source of truth), `rgx_audit_lua` (deterministic unsafe-pattern detectors), `rgx_generate_addon` (shipped keys only), `rgx_get_contract` + `rgx://` resources. Registered in `.mcp.json`, so Claude Code sessions here get the `rgx_*` tools (run `npm install` in `tools/rgx-mcp/` once). Dependency direction enforced: the tool reads RGX docs/schema; the addon runtime never references `tools/`. The old separate repo `DonnieDice/rgx-mcp` is archived with a moved notice. Tier 4 implementation should update the schema annotations (`tier4` → `today`) as each key lands — nothing else changes.
