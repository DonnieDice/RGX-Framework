...
[`v2.7.2`](https://github.com/DonnieDice/RGX-Framework/releases/tag/v2.7.2)
publishes exactly two GitHub assets:

| Asset | Purpose |
|---|---|
| `RGX-Framework-v2.7.2.zip` | The only product archive; install this addon |
| `release.json` | BigWigs packager metadata for automation |

The inspected `v2.7.2` ZIP contains one `RGX-Framework/` root and exactly 100
runtime files. GitHub records the digest on the release asset. To query it with
GitHub CLI:

```bash
gh release view v2.7.2 --repo DonnieDice/RGX-Framework --json assets \
  --jq '.assets[] | select(.name == "RGX-Framework-v2.7.2.zip") | .digest'
```
