...
15. ✅ **`rgx-mcp` v0.1.0 transition fixture** — lives only in the source repo at `tools/rgx-mcp/` for contract-conformance CI and never ships in Framework releases. Run `npm ci` in that directory. Public MCP/API/editor tooling belongs to the future RGX Studio product. The fixture reads the canonical Framework schema/docs and its RGX-Hello E2E must be rerun after contract changes; the addon runtime never references `tools/`.
